...
class GeminiDiagnosisProvider(CloudDiagnosisProvider):
    ...
    async def analyze_sensor(self, request: SensorAnalysisRequest) -> SensorAnalysisRawResult:
        ...
        awf_info = f"Plant-Available Water Fraction (AWF): {int(request.available_water_fraction * 100)}%" if request.available_water_fraction is not None else ""
        fc_pwp_info = f"Field Capacity (FC): {request.field_capacity}%, Wilting Point (PWP): {request.wilting_point}%" if request.field_capacity and request.wilting_point else ""
        adc_info = f"Raw ADC Response: {request.raw_adc}" if request.raw_adc else ""

        system_prompt = (
            "You are an expert agronomist and soil scientist for the AgriX agricultural intelligence platform.\n"
            "Analyze real-time sensor telemetry, soil context, and plant-available water alongside the detected crop and disease context to provide a unified, action-oriented recommendation.\n\n"
            "STRICT RULES:\n"
            "1. Focus on what the farmer should do TODAY answering WHAT, WHY, WHEN, and ACTION.\n"
            "2. Interpret plant-available water fraction: <25% (Very Dry/Irrigate now), 25-50% (Depleting/Plan irrigation), 50-75% (Adequate/Maintain), >75% (Saturated/Pause).\n"
            "3. If a disease is detected, combine humidity & moisture readings to provide preventive advice without guaranteeing disease emergence.\n"
            "4. Do not make unrealistic yield increase percentage promises; focus on practical growth and moisture management.\n"
            "5. Return strictly valid JSON matching the exact schema below, with no markdown code blocks.\n\n"
            "Response Schema:\n"
            "{\n"
        ...
        user_content = (
            ...
            f"- Soil Type & Hydrology: {soil_info} ({fc_pwp_info})\n"
            f"- Temperature: {request.temperature} °C\n"
            f"- Humidity: {request.humidity} %\n"
            f"- Estimated VWC Moisture: {request.soil_moisture} %\n"
            f"- {awf_info}\n"
            f"- {adc_info}\n"
